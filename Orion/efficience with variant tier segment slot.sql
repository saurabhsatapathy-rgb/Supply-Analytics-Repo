...
campaign_run_variant AS (
    SELECT DISTINCT campaign_id, run_date, variant
...
exploded AS (
  SELECT
    ...
    ps.slot,
    ps.emit_segment,
    p.ctx.burn.actual_cdgmv_pct,
    p.ctx.burn.agreed_cdgmv_pct,
    p.ctx.burn.cumulative_gmv,
    p.ctx.burn.cumulative_rdgmv,
    p.ctx.burn.cumulative_sdgmv,
    p.ctx.burn.lower_cdgmv_tol,
    p.ctx.burn.total_burn,
    p.ctx.burn.upper_cdgmv_tol
  FROM parsed p
  LEFT JOIN campaign_run_variant v
    ON v.campaign_id = p.campaign_id AND v.run_date = p.run_date
  LATERAL VIEW explode(p.ctx.stores) AS store
...
select run_date,
a.store_id, tier, mode, segment, a.slot, a.variant,
...
where run_date between '2026-08-19' and '2026-08-25'
) a
left join
(select dt, 
    store_id, 
    ...
    where to_date(a.dt) >= to_date('2026-08-19')
    and to_date(a.dt) <= to_date('2026-08-25')
